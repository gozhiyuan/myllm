...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            print(len(paths), paths[0].keys())
            print(paths[0]['observation'].shape, paths[0]['action'].shape)
            print(paths[0]['reward'].shape, paths[0]['next_observation'].shape)
            print(paths[0]['terminal'].shape)
