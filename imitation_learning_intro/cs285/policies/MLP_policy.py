...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        """
        # Get the mean from the MLP network
        mean = self.mean_net(observation)
        
        # Create a normal distribution with learned mean and standard deviation
        std = torch.exp(self.logstd)
        distribution = distributions.Normal(mean, std)
        
        # Return the distribution object which can be sampled from and is differentiable
        return distribution
    ...
    def update(self, observations, actions):
        ...
        """
        # Convert observations and actions to tensors if they aren't already
